"""Firebase ID token verification + caching.

Verifies the bearer token on every authenticated request. Spec §5.1.

In v1 we cache the verified user UID against the token's hash for ~1h,
matching Firebase token expiry. The cache lives in Redis (key
`firebase:token:{kid}` per spec §6.3).
"""
...
class AuthContext:
    """The authenticated principal — passed into request handlers."""
